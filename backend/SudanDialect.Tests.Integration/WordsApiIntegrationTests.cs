...
public sealed class WordsApiIntegrationTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task GetById_ShouldIncrementVisitCount_WhenCalled()
    {
        _factory.Should().NotBeNull();
        _client.Should().NotBeNull();
        ...
        await _client!.GetAsync($"/api/words/{publicId}", TestContext.Current.CancellationToken);
        await _client!.GetAsync($"/api/words/{publicId}", TestContext.Current.CancellationToken);

        // Assert
        var updatedWord = await db.Words.AsNoTracking().SingleAsync(w => w.Id == word.Id, TestContext.Current.CancellationToken);
        updatedWord.VisitCount.Should().Be(2);
        updatedWord.LastVisitedAt.Should().NotBeNull();
        updatedWord.LastVisitedAt.Value.Should().BeCloseTo(DateTime.UtcNow, TimeSpan.FromSeconds(5));
    }

    [Fact]
    public async Task GetVisitsPagedAsync_ShouldReturnWordsSortedByVisitCount()
    {
        _factory.Should().NotBeNull();

        await using var scope = _factory!.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        var adminRepo = scope.ServiceProvider.GetRequiredService<IAdminWordRepository>();

        // Arrange
        var word1 = new Word
        {
            Headword = "كلمة 1",
            NormalizedHeadword = "كلمة 1",
            Definition = "تعريف 1",
            NormalizedDefinition = "تعريف 1",
            VisitCount = 10,
            IsActive = true
        };
        var word2 = new Word
        {
            Headword = "كلمة 2",
            NormalizedHeadword = "كلمة 2",
            Definition = "تعريف 2",
            NormalizedDefinition = "تعريف 2",
            VisitCount = 50,
            IsActive = true
        };
        var word3 = new Word
        {
            Headword = "كلمة 3",
            NormalizedHeadword = "كلمة 3",
            Definition = "تعريف 3",
            NormalizedDefinition = "تعريف 3",
            VisitCount = 5,
            IsActive = true
        };

        db.Words.AddRange(word1, word2, word3);
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);

        // Act - Sort by visitCount desc
        var (items, totalCount) = await adminRepo.GetVisitsPagedAsync("visitCount", true, 1, 10, TestContext.Current.CancellationToken);

        // Assert
        totalCount.Should().BeGreaterThanOrEqualTo(3);
        items[0].VisitCount.Should().Be(50);
        items[1].VisitCount.Should().Be(10);
        items[2].VisitCount.Should().Be(5);
    }
