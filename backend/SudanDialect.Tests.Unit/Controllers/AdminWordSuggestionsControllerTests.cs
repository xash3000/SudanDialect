using FluentAssertions;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Moq;
using SudanDialect.Api.Controllers;
using SudanDialect.Api.Dtos.Admin;
using SudanDialect.Api.Interfaces.Services;

namespace SudanDialect.Tests.Unit.Controllers;

public class AdminWordSuggestionsControllerTests
{
    private readonly Mock<IAdminWordSuggestionService> _adminWordSuggestionServiceMock;
    private readonly AdminWordSuggestionsController _sut;

    public AdminWordSuggestionsControllerTests()
    {
        _adminWordSuggestionServiceMock = new Mock<IAdminWordSuggestionService>();
        _sut = new AdminWordSuggestionsController(_adminWordSuggestionServiceMock.Object);

        _sut.ControllerContext = new ControllerContext
        {
            HttpContext = new DefaultHttpContext()
        };
    }

    [Fact]
    public async Task GetPage_ShouldReturnOk_WithPageResults()
    {
        // arrange
        var query = new AdminWordSuggestionQueryDto();
        var expectedPage = new AdminWordSuggestionPageDto
        {
            Items = new List<AdminWordSuggestionItemDto>(),
            Page = 1,
            PageSize = 10,
            TotalCount = 0,
            TotalPages = 0
        };

        _adminWordSuggestionServiceMock
            .Setup(s => s.GetPageAsync(query, It.IsAny<CancellationToken>()))
            .ReturnsAsync(expectedPage);

        // act
        var actionResult = await _sut.GetPage(query, TestContext.Current.CancellationToken);

        // assert
        actionResult.Result.Should().NotBeNull();
        var okResult = actionResult.Result.Should().BeOfType<OkObjectResult>().Subject;
        okResult.StatusCode.Should().Be(200);
        okResult.Value.Should().BeEquivalentTo(expectedPage);
    }

    [Fact]
    public async Task GetPage_ShouldThrowArgumentException_WhenInvalidQuery()
    {
        // arrange
        var query = new AdminWordSuggestionQueryDto();
        _adminWordSuggestionServiceMock
            .Setup(s => s.GetPageAsync(query, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ArgumentException("Invalid"));

        // act & assert
        await Assert.ThrowsAsync<ArgumentException>(() =>
            _sut.GetPage(query, TestContext.Current.CancellationToken));
    }

    [Fact]
    public async Task SetResolved_ShouldReturnOk_WhenUpdated()
    {
        // arrange
        var id = 1;
        var request = new AdminSetWordSuggestionResolvedRequestDto { Resolved = true };

        _adminWordSuggestionServiceMock
            .Setup(s => s.SetResolvedAsync(id, request.Resolved, It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);

        // act
        var actionResult = await _sut.SetResolved(id, request, TestContext.Current.CancellationToken);

        // assert
        actionResult.Result.Should().NotBeNull();
        var okResult = actionResult.Result.Should().BeOfType<OkObjectResult>().Subject;
        okResult.StatusCode.Should().Be(200);
    }

    [Fact]
    public async Task SetResolved_ShouldReturnNotFound_WhenNotUpdated()
    {
        // arrange
        var id = 1;
        var request = new AdminSetWordSuggestionResolvedRequestDto { Resolved = true };

        _adminWordSuggestionServiceMock
            .Setup(s => s.SetResolvedAsync(id, request.Resolved, It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);

        // act
        var actionResult = await _sut.SetResolved(id, request, TestContext.Current.CancellationToken);

        // assert
        actionResult.Result.Should().NotBeNull();
        actionResult.Result.Should().BeOfType<NotFoundResult>();
    }

    [Fact]
    public async Task SetResolved_ShouldThrowArgumentException_WhenInvalidRequest()
    {
        // arrange
        var id = 1;
        var request = new AdminSetWordSuggestionResolvedRequestDto { Resolved = true };

        _adminWordSuggestionServiceMock
            .Setup(s => s.SetResolvedAsync(id, request.Resolved, It.IsAny<CancellationToken>()))
            .ThrowsAsync(new ArgumentException("Invalid"));

        // act & assert
        await Assert.ThrowsAsync<ArgumentException>(() =>
            _sut.SetResolved(id, request, TestContext.Current.CancellationToken));
    }
}
